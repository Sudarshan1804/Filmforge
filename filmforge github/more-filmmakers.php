...
<head>
    ...
    <title>FilmForge - More Filmmakers</title>
...
<body class="font-inter bg-gray-100 dark:bg-gray-900 transition-colors duration-300 pt-16">
    ...
    <section class="py-16 animate-on-scroll">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-900 dark:text-gray-100 text-center text-shadow">More Filmmakers</h2>
            <div class="mt-12 grid gap-8 lg:grid-cols-3 sm:grid-cols-2">
                <?php
                if (isset($_SESSION['user_id'])) {
                    try {
                        $stmt = $pdo->prepare("SELECT id, name, bio, profile_photo FROM users WHERE id != ?");
                        $stmt->execute([$_SESSION['user_id']]);
                        while ($user = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ?>
                            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 text-center filmmaker-card animate__animated animate__bounceIn">
                ...
                <?php
                        ...
                        echo "<div class='text-center text-red-600 dark:text-red-400'>Error loading filmmakers: " . htmlspecialchars($e->getMessage()) . "</div>";
                    }
                }
                ?>
            </div>
            <div class="mt-8 text-center">
                <a href="index.php#filmmakers" class="inline-block bg-indigo-600 dark:bg-indigo-500 text-white px-6 py-3 rounded-md text-sm font-medium hover:bg-indigo-700 dark:hover:bg-indigo-600 btn-animated">Back to Filmmakers</a>
            </div>
        </div>
    </section>
